...
from app.services import caf_data, caf_pdf_fill
from app.services.lease_service import LeaseService
from app.utils.file_handler import get_file_path, save_bytes
...
_DOC_DOCTYPE = {
    "attestation": DocumentType.ATTESTATION_CAF,
    "tiers_payant": DocumentType.ATTESTATION_TIERS,
}
...
def _tpl_out(tpl: CafTemplate | None, fields: list | None = None) -> dict:
    return {
        ...
        else (caf_pdf_fill.extract_fields(_read_tpl(tpl)) if tpl else []),
...
def _read_tpl(tpl: CafTemplate | None) -> bytes:
    if not tpl:
        return b""
...
async def _get_tpl(db: AsyncSession, gid, doc_type: str) -> CafTemplate | None:
    return (
        await db.execute(
            select(CafTemplate).where(
                CafTemplate.gestionnaire_id == gid, CafTemplate.doc_type == doc_type
            )
        )
    ).scalar_one_or_none()
...
class MappingIn(BaseModel):
    field_map: dict
    sign_page: int | None = None
    sign_x_mm: int | None = None
    sign_y_mm: int | None = None
    sign_w_mm: int | None = None
...
async def save_mapping(
    ...
    clean = {
        str(k): str(v)
        for k, v in (data.field_map or {}).items()
...
def _guess_key(field_name: str) -> str | None:
...
async def _build_pdf(
    db: AsyncSession, current_user: User, lease_id: uuid.UUID, doc_type: str
) -> tuple[bytes, str]:
    ...
    if doc_type == "attestation":
        resp = await _letters.attestation_caf(
            lease_id, db=db, current_user=current_user, _feat=current_user
        )
    else:
        resp = await _letters.versement_direct_caf(
            lease_id, db=db, current_user=current_user, _feat=current_user
        )
...
async def deposit_pdf(
    ...
    await DocumentService.save_generated(
        ...
        document_type=_DOC_DOCTYPE[doc_type],
...
async def email_pdf(
    ...
    label = _DOC_LABEL[doc_type]
    html = (
        f"<p>Bonjour {tenant.full_name},</p>"
