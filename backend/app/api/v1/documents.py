...
async def list_documents(
    ...
    if role == Role.LOCATAIRE:
        ...
        if not tenant:
            return []
        leases = (
            (await db.execute(select(Lease).where(Lease.tenant_id == tenant.id))).scalars().all()
        )
    ...
    if role == Role.GESTIONNAIRE_PROPRIO:
        ...
        prop_ids = (
            (await db.execute(select(Property.id).where(Property.created_by == current_user.id)))
            .scalars()
            .all()
        )
