...
from app.services.signalement_service import (
    SignalementService, enrich, CATEGORY_LABELS, URGENCY_LABELS, STATUS_LABELS,
)
...
async def export_csv(
    ...
    w.writerow(["Date", "Survenu le", "Catégorie", "Urgence", "Statut", "Source",
                "Bien", "Locataire", "Description"])
    for s in items:
...
async def list_signalements(
    ...
    items, total = await SignalementService.list_for_manager(
        db, current_user, status=status, category=category, urgency=urgency,
