...
from app.schemas.tenant import TenantCreate, TenantListItem, TenantResponse, TenantUpdate
...
async def list_tenants(
    search: str | None = Query(None, description="Recherche par nom, email, téléphone"),
    skip: int = Query(0, ge=0),
    limit: int = Query(50, ge=1, le=200),
    available_only: bool = Query(
        False, description="Exclure les locataires ayant déjà un bail actif"
    ),
    ...
    if Role(current_user.role) == Role.GESTIONNAIRE_PROPRIO:
                ...
                for tid in (
                    await db.execute(
                        select(Lease.tenant_id).where(
                            Lease.property_id.in_(prop_ids), Lease.tenant_id.isnot(None)
                        )
                    )
                )
                .scalars()
        ...
        own = [
            t for t in all_tenants if t.created_by == current_user.id or t.id in lease_tenant_ids
        ]
        if available_only:
            active_ids = {
                l.tenant_id
