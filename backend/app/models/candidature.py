...
class Candidature(Base, TimestampMixin):
    ...
    employment: Mapped[str | None] = mapped_column(String(150), nullable=True)  # situation pro
    ...
    docs: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    # Jeton public pour le dépôt des pièces par le candidat (lien envoyé par e-mail).
    upload_token: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    # Visite : créneau réservé par le candidat + date d'invitation envoyée.
    visit_slot_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("property_visit_slots.id", ondelete="SET NULL"),
        nullable=True,
    )
    visit_invited_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    # Relance avant visite déjà envoyée (anti-doublon de la relance automatique).
    visit_reminded_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    # Relance « dossier incomplet » déjà envoyée (anti-doublon de la relance auto).
    docs_reminded_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    # Rappel interne « dossier en étude sans suite » déjà poussé (anti-doublon).
