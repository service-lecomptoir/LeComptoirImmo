...
class User(Base, TimestampMixin):
    ...
    address: Mapped[str | None] = mapped_column(String(300), nullable=True)  # rue (n° + voie)
    ...
    owner_kind: Mapped[str] = mapped_column(
        String(10), nullable=False, default="personne", server_default="personne"
    )
    ...
    owner_full_name: Mapped[str | None] = mapped_column(String(150), nullable=True)
    ...
    owner_national_id: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    email_theme: Mapped[str | None] = mapped_column(String(20), nullable=True)
    ...
    proprio_visibility: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    # Sur le compte GESTIONNAIRE (racine d'agence) : défaut appliqué à tous ses
    # propriétaires qui n'ont pas de surcharge.
    proprio_visibility_default: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    ...
    sumup_api_key_enc: Mapped[str | None] = mapped_column(Text, nullable=True)
    sumup_merchant_code: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    @property
    def bailleur_name(self) -> str | None:
