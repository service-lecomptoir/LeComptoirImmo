...
async def build_values(db: AsyncSession, current_user, lease) -> dict:
    ...
    return {
        ...
        "bailleur_phone": (owner.phone if owner else None) or (prop.owner_phone if prop else None) or getattr(current_user, "phone", "") or "",
        "bailleur_email": (owner.email if owner else None) or (prop.owner_email if prop else None) or getattr(current_user, "email", "") or "",
        ...
        "area_sqm": f"{float(prop.area_sqm):.0f}" if prop and getattr(prop, "area_sqm", None) else "",
        "rent_no_charges": f"{float(lease.rent_amount):.2f}" if lease.rent_amount is not None else "",
        "charges": f"{float(lease.charges_amount):.2f}" if lease.charges_amount is not None else "",
        "total_tcc": f"{lease.total_monthly:.2f}" if getattr(lease, "total_monthly", None) is not None else "",
