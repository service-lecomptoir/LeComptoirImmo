...
def _data_uri_to_png(uri: Optional[str]) -> Optional[bytes]:
...
def _signature_overlay(width: float, height: float, png: bytes,
                       x_mm: float, y_mm: float, w_mm: float) -> Optional[bytes]:
    ...
    try:
        ...
        c.drawImage(img, x_mm * mm, y_mm * mm, width=w_pt, height=h_pt,
