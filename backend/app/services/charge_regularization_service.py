...
class ChargeRegularizationService:
    ...
    async def provisions_paid(
        ...
        rows = (
            await db.execute(
                select(Payment.period_year, Payment.period_month, Payment.amount_charges).where(
    ...
    async def apply(
        ...
        rev = await RentRevisionService.schedule(
            ...
            source="charges",
            reason="Régularisation des charges",
            created_by=created_by,
    ...
    async def update(
        ...
        if rev:
            ...
            rev = await RentRevisionService.schedule(
                ...
                source="charges",
                reason="Régularisation des charges (modifiée)",
                notify=False,  # l'e-mail (avec décompte) est envoyé par _notify
            )
            reg.rent_revision_id = rev.id
        await db.flush()
    ...
    async def delete(db: AsyncSession, reg: ChargeRegularization, lease: Lease) -> None:
        ...
        lease.charges_amount = round(
            RentRevisionService._effective_field(
                float(reg.old_monthly_provision), remaining, "charges", date.today()
            ),
            2,
        )
    ...
    async def _notify(db: AsyncSession, lease: Lease, reg: ChargeRegularization) -> None:
        ...
        try:
            ...
            if bal > 0:
                solde = (
                    f"Un trop-perçu de {bal:.2f} € vous est restitué "
            ...
            if email:
                ...
                rev = (
                    await db.get(RentRevision, reg.rent_revision_id)
                    if reg.rent_revision_id
                    else None
                )
