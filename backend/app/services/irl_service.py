...
class IrlService:
    ...
    @staticmethod
    async def get_index(db: AsyncSession, year: int, quarter: int) -> Optional[IrlIndex]:
        return (await db.execute(
            select(IrlIndex).where(IrlIndex.year == year, IrlIndex.quarter == quarter)
        )).scalar_one_or_none()

    @staticmethod
    async def get_latest_for_quarter(db: AsyncSession, quarter: int) -> Optional[IrlIndex]:
    ...
    @staticmethod
    async def upsert(db: AsyncSession, year: int, quarter: int, value: float,
                     source: str = "manuel") -> IrlIndex:
        ...
        try:
            ...
            for obs in observations:
                ...
                try:
                    ...
                    continue
            await db.flush()
            return {"fetched": fetched, "configured": True,
