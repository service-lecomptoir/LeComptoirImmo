...
async def export_tenant(db: AsyncSession, tenant: Tenant) -> dict:
    ...
    leases = list((await db.execute(select(Lease).where(Lease.tenant_id == tenant.id))).scalars())
    payments = list(
        (await db.execute(select(Payment).where(Payment.tenant_id == tenant.id))).scalars()
    )
    avis = list(
        (
            await db.execute(select(AvisEcheance).where(AvisEcheance.tenant_id == tenant.id))
        ).scalars()
    )
    tickets = list(
        (await db.execute(select(Ticket).where(Ticket.tenant_id == tenant.id))).scalars()
    )
    documents = list(
        (
            await db.execute(
                select(Document).where(
                    Document.entity_type == "tenant", Document.entity_id == tenant.id
                )
            )
        ).scalars()
    )
...
async def anonymize_tenant(db: AsyncSession, tenant: Tenant) -> dict:
    ...
    tenant.notes = None
    tenant.anonymized_at = datetime.now(UTC)
...
async def apply_retention(
    ...
    *,
    ...
    tenants = list(
        (await db.execute(select(Tenant).where(Tenant.anonymized_at.is_(None)))).scalars()
    )
    tenant_done = 0
    for t in tenants:
        leases = list((await db.execute(select(Lease).where(Lease.tenant_id == t.id))).scalars())
