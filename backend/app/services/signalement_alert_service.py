...
def is_night(dt: datetime | None) -> bool:
...
class SignalementAlertService:
    ...
    async def process_new(db: AsyncSession, s: Signalement) -> None:
        ...
        try:
            ...
            if is_night(s.occurred_at):
                tenant_uids = await SignalementAlertService._property_tenant_user_ids(
                    db, s.property_id
                )
                for uid in tenant_uids:
                    SignalementAlertService._notify(
                        db,
                        uid,
                        "Nuisance sonore signalée la nuit",
                        _COURTESY_MSG,
                        NotificationPriority.NORMAL,
                        s.id,
                    )
                    SignalementAlertService._log(
                        db, AlertType.NOCTURNE.value, s.property_id, s.id, uid, _COURTESY_MSG
                    )
                if not tenant_uids:
                    # Aucun compte locataire : on journalise quand même l'alerte nocturne.
                    SignalementAlertService._log(
                        db, AlertType.NOCTURNE.value, s.property_id, s.id, None, _COURTESY_MSG
                    )
    ...
    async def _maybe_escalate(db: AsyncSession, s: Signalement) -> None:
        ...
        msg = (
            f"Le bien « {name} » fait l'objet de {count} signalements de bruit sur "
            f"{ESCALATION_WINDOW_DAYS} jours. Une action de votre part est recommandée."
        )
        SignalementAlertService._notify(
            db,
            manager_id,
            "Bruit récurrent : action recommandée",
            msg,
            NotificationPriority.HIGH,
            s.id,
        )
        SignalementAlertService._log(
            db, AlertType.ESCALADE.value, s.property_id, s.id, manager_id, msg
        )
    ...
    async def run_preventive_reminders(db: AsyncSession) -> int:
        ...
        for pid, _n in targets:
            ...
            for uid in uids:
                SignalementAlertService._notify(
                    db, uid, "Rappel de bon voisinage", _PREVENTIVE_MSG, NotificationPriority.LOW
                )
            SignalementAlertService._log(
                db, AlertType.PREVENTIF.value, pid, None, uids[0] if uids else None, _PREVENTIVE_MSG
            )
            done += 1
        return done
