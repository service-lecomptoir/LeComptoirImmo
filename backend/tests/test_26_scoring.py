...
@pytest.mark.asyncio
class TestScoring:
    async def test_list_and_detail(self, client, gp_token):
        tenant_id, _ = await _chain(client, gp_token)
        lst = await client.get("/api/v1/scoring", headers=auth(gp_token))
        ...
        det = await client.get(f"/api/v1/scoring/{tenant_id}", headers=auth(gp_token))
        assert det.status_code == 200, det.text
        data = det.json()
        assert {"score", "grade", "strategy", "factors", "stats"} <= set(data)
        assert any(f["key"] == "ponctualite" for f in data["factors"])

    async def test_relationship_event_lowers_relation_score(self, client, gp_token):
        tenant_id, lease_id = await _chain(client, gp_token)
        before = (await client.get(f"/api/v1/scoring/{tenant_id}", headers=auth(gp_token))).json()
        ...
        after = (await client.get(f"/api/v1/scoring/{tenant_id}", headers=auth(gp_token))).json()
    ...
    async def test_event_kinds_listed(self, client, gp_token):
        resp = await client.get("/api/v1/scoring/event-kinds", headers=auth(gp_token))
    ...
    async def test_mandataire_cannot_access_scoring(self, client, gestionnaire_token):
        # Le scoring locataires est réservé au gestionnaire propriétaire (et admin).
        assert (
            await client.get("/api/v1/scoring", headers=auth(gestionnaire_token))
        ).status_code == 403

    async def test_invalid_event_kind_rejected(self, client, gp_token):
        _, lease_id = await _chain(client, gp_token)
        resp = await client.post(f"/api/v1/leases/{lease_id}/relationship-events",
                                 headers=auth(gp_token), json={"kind": "n_existe_pas"})
