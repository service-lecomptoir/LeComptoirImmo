...
async def login(
    ...
    """Authentification admin ProxyGen."""
    result = await db.execute(
        select(ProxygenAdmin).where(ProxygenAdmin.email == request.email)
    )
...
async def me(
    current_admin: ProxygenAdmin = Depends(get_current_proxygen_admin),
):
