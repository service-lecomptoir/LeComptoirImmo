...
function CreateModal({ plans, onClose, onCreated }: CreateModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(axiosError?.response?.data?.detail || 'Erreur lors de la creation')
    } finally {
      setSaving(false)
    }
  }
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Cree le</th>
              </tr>
            </thead>
